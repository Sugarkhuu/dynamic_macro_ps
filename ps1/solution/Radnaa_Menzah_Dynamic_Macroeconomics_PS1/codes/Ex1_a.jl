...
gri   = (
            k = exp.(range(0,log(mpar.maxk-mpar.mink+1),length = mpar.nk)) .- 1 .+mpar.mink,
            z = [1/9; 10/9]
        )
...
if par.γ == 1.0
    ...
    util(c)      = 1 ./(1-par.γ) .*c .^(1-par.γ) # Utility
    mutil(c)     = 1 ./(c .^par.γ) # Marginal utility
    invmutil(mu) = 1 ./(mu .^(1/par.γ)) # inverse marginal utility
end
