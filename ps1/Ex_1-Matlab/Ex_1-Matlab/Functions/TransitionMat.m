...
[~,idk]                  = histc(xxx,xxx); % find the next lowest point on grid for policy
...
for zz=1:mpar.nz % all current income states
    for kk=1:mpar.nk % all current asset states
        Trans_array(kk,zz,xxx,:)   =  xxx *reshape(P(zz,:),[1 1 1 mpar.nz]); % probability to move left  to optimal policy choice
        Trans_array(kk,zz,xxx,:)   =  xxx *reshape(P(zz,:),[1 1 1 mpar.nz]); % probability to move right to optimal policy choice
    end
end
